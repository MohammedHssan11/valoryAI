...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'ck_areas_level'
      AND conrelid = 'areas'::regclass
  ) AND NOT EXISTS (
    SELECT 1 FROM areas WHERE level NOT BETWEEN 1 AND 4
  ) THEN
    ALTER TABLE areas
      ADD CONSTRAINT ck_areas_level CHECK (level BETWEEN 1 AND 4);
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    SELECT 1 FROM listings WHERE area_id IS NULL
  ) THEN

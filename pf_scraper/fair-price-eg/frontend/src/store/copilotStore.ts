...
function buildToolInputs(): Partial<Record<CopilotPlannedToolCall, CopilotToolInput>> {
  ...
  const toolInputs: Partial<Record<CopilotPlannedToolCall, CopilotToolInput>> = {
    MARKET_INSIGHT_TOOL: {
      ...
      property_type: market.filters.property_type || valuation.lastRequest?.property_type || null,
    },
  };
  ...
  if (typeof propertyId === "number") {
    ...
    const askingPrice = targetPrice(valuation.lastRequest);
    if (askingPrice != null) {
  ...
  return {
    activeProperty:
      typeof propertyContext.activePropertyId === "number"
        ? `${request?.property_type ?? "Property"} at ${location} (Property ${propertyContext.activePropertyId})`
...
export const useCopilotStore = create<CopilotState>((set) => ({
  ...
  sendMessage: async ({ message, brokerSessionId = null, signal }) => {
    ...
    if (typeof workspaceId !== "number") {
      const error = new Error("Run a valuation first so Copilot has an active property workspace.");
      set({ error });
      throw error;
    }

...
| Risk | Severity | Notes |
|---|---|---|
| No authentication | CRITICAL | Any exposed API is publicly callable. |
| No authorization/RBAC | CRITICAL | No separation between investor/admin/broker. |
...
Security production readiness: 35%
...
Readiness classification: Staging-capable prototype with a strong deterministic valuation core; not production-enterprise ready.
...
- Persist broker sessions and valuation history.
- Refine broker staged reasoning timeline, partial narration presentation, and governance overlays.
- Add `/v1/broker/session/{session_id}` UI retrieval for analytical continuity.
- Add optional WebSocket streaming design only if SSE proves insufficient.
- Add map/location picker to valuation form.
- Add operational dashboard wiring for `/health/operational`.
- Add Alembic or explicit migration runner.
- Add Redis or external limiter/cache design.
...
1. Security foundation: authn/authz, tenant model, API protection.

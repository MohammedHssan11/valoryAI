...
export function ValuationScreen() {
  ...
  const preparedContextSignatureRef = React.useRef<string | null>(null);

  React.useEffect(() => {
    if (lastResult || !isPropertyContextReady(draft)) return;

    const signature = propertyContextSignatureKey(draft);
    if (preparedContextSignatureRef.current === signature && bridgeStatus === "ready") return;

    const controller = new AbortController();
    const timeout = window.setTimeout(() => {
      preparedContextSignatureRef.current = signature;
      setBridgePending(`property_context_${signature.slice(0, 24)}`);
      void preparePropertyContext({
        request: {
          ...draft,
        ...
        .catch((bridgeError) => {
          if (controller.signal.aborted || isCancelledBridge(bridgeError)) return;
          preparedContextSignatureRef.current = null;
          setBridgeError(bridgeErrorMessage(bridgeError));
        });
    }, 500);

...
export function propertyContextSignatureKey(request: RentFairPriceRequest): string {
  return propertySignature(request);
}

export function isPropertyContextReady(request: RentFairPriceRequest): boolean {
  const hasLocation =
    firstString(request.address, request.compound_name, request.canonical_entity_id) !== undefined ||
    (typeof request.lat === "number" && typeof request.lng === "number");
  return hasLocation && typeof request.size_sqm === "number" && request.size_sqm > 0 && firstString(request.property_type) !== undefined;
}
...
async function createPropertyContext(
  ...
  request: RentFairPriceRequest,
  valuation: RentFairPriceData | undefined,
  signal?: AbortSignal,
...
export async function bridgeValuationToPropertyContext(input: PropertyContextBridgeInput): Promise<PropertyContextBinding> {
  ...
  return {
    ...
    contextSource: "valuation",
  };
}

export async function preparePropertyContext(input: PropertyContextPrepareInput): Promise<PropertyContextBinding> {
  const workspaces = await listWorkspaces(input.signal);
  const selected = selectWorkspace(workspaces, input.preferredWorkspaceId);
  const workspace = selected.workspace ?? (await createWorkspace(input.signal));
  const properties = await listProperties(workspace.id, input.signal);
  const matchedProperty = findMatchingProperty(properties, input.request);
  const property = matchedProperty ?? (await createPropertyContext(workspace, input.request, undefined, input.signal));

...
class _CopilotScreenState extends State<CopilotScreen> {
  ...
  late final CopilotRemoteDataSource _copilotRemoteDataSource;
  ...
  void initState() {
    super.initState();
    _copilotRemoteDataSource = CopilotRemoteDataSource();
    _repository = CopilotRepositoryImpl(_copilotRemoteDataSource);
  ...
  Future<void> _sendMessage(String text) async {
    ...
    try {
      ...
      final propertyContextId = await _prepareActivePropertyContext(
        activeWorkspaceId,
      );
      final toolInputs = _buildToolInputs(
        activeWorkspaceId,
  ...
  Future<int?> _prepareActivePropertyContext(int workspaceId) async {
    final context = _activeValuationContext;
    if (context == null) {
      return _activePropertyContextId;
    }

    final raw = context.rawData;
    final existingPropertyId = _intFromRaw(raw, const [
      'property_id',
      'property_state_id',
    ]);
    if (existingPropertyId != null) {
      _activePropertyContextId = existingPropertyId;
      return existingPropertyId;
    }

    final request = _valuationRequestPayload(raw);
    if (request == null) {
      return _activePropertyContextId;
    }

    final signature = _propertySignature(request);
    final properties = await _copilotRemoteDataSource.getWorkspaceProperties(
      workspaceId,
    );
    Map<String, dynamic>? property;
    for (final candidate in properties) {
      final inputs = candidate['valuation_inputs'];
      if (inputs is Map && _propertySignature(Map<String, dynamic>.from(inputs)) == signature) {
        property = candidate;
        break;
      }
    }

    property ??= await _copilotRemoteDataSource.createPropertyContext(
      workspaceId: workspaceId,
      property: _propertyPayloadFromRequest(request, raw),
    );
    ...
    _activePropertyContextId = propertyId;
    raw['property_id'] = propertyId;

    final requestId = _stringFromRaw(raw, const [
      '_valuation_request_id',
      'request_id',
      'valuation_id',
    ]);
    if (requestId != null) {
      await _copilotRemoteDataSource.recordValuationContextEvent(
        workspaceId: workspaceId,
        propertyId: propertyId,
        valuationRequest: request,
        valuationResult: _valuationResultPayload(raw),
        requestId: requestId,
      );
      raw['valuation_id'] = requestId;
  ...
  Map<String, dynamic>? _valuationRequestPayload(Map<String, dynamic> raw) {
    final request = raw['_valuation_request'];
    if (request is Map) {
  ...
  Map<String, dynamic> _valuationResultPayload(Map<String, dynamic> raw) {
    final result = Map<String, dynamic>.from(raw);
    result.remove('_valuation_request');
    result.remove('_valuation_request_id');
    return result;
  }

  Map<String, dynamic> _propertyPayloadFromRequest(
    Map<String, dynamic> request,
    Map<String, dynamic> valuation,
  ) {
    final propertyType = _stringFromMap(request, 'property_type') ?? 'Property';
    final location = _propertyLocation(request, valuation);
    return {
      'label': '$propertyType - $location',
      'location': location,
      'area': _numFromMap(request, 'size_sqm') ?? 1,
      'bedrooms': _intFromMap(request, 'bedrooms') ?? 0,
      'bathrooms': _intFromMap(request, 'bathrooms') ?? 0,
      'amenities': {
        'codes': _stringListFromMap(request, 'amenities')..sort(),
      },
      'property_type': propertyType,
      'property_category':
          _stringFromMap(request, 'property_category') ??
          _stringFromMap(valuation, 'property_category') ??
          'residential_rent',
      'valuation_inputs': request,
    };
  }
  ...
  String _propertyLocation(
    ...
    final explicit = _stringFromMap(request, 'address') ??
        _stringFromMap(request, 'compound_name') ??
        _stringFromMap(valuation, 'compound_name');
    if (explicit != null) {
      return explicit;
    }
    final area = valuation['area'];
    if (area is Map) {
      final areaName = _stringFromMap(Map<String, dynamic>.from(area), 'name') ??
          _stringFromMap(Map<String, dynamic>.from(area), 'area_name');
      if (areaName != null) {
        return areaName;
      }
    }
    final lat = _numFromMap(request, 'lat');
    final lng = _numFromMap(request, 'lng');
  ...
  String _propertySignature(Map<String, dynamic> request) {
    final normalized = Map<String, dynamic>.from(request)
      ..remove('target_price_egp');
    return jsonEncode(_stableJson(normalized));
  }
  ...
  Object? _stableJson(Object? value) {
    ...
    if (value is Map) {
      ...
      for (final key in sorted.keys.map((key) => key.toString()).toList()..sort()) {
        final entry = sorted[key];
        if (entry != null) {
          result[key] = _stableJson(entry);
        }
      }
      return result;
    }
  ...
  Map<String, dynamic>? _buildToolInputs(
    int workspaceId, {
    int? propertyContextId,
  }) {
    final context = _activeValuationContext;
    if (context == null) {
      return null;
    ...
    final propertyId = propertyContextId ??
        _intFromRaw(raw, const [
          'property_id',
          'property_state_id',
        ]);
    ...
    final request = _valuationRequestPayload(raw);
    final askingPrice =
        (request == null ? null : _intFromMap(request, 'target_price_egp')) ??
        _intFromRaw(raw, const ['fair_price_egp']);
  ...
  Map<String, dynamic>? _marketInsightInput(
    ...
    final request = _valuationRequestPayload(raw);
    final propertyType =
        _stringFromRaw(raw, const ['property_type']) ??
        (request == null ? null : _stringFromMap(request, 'property_type'));
    final compoundName =
        _stringFromRaw(raw, const ['compound_name']) ??
        (request == null ? null : _stringFromMap(request, 'compound_name'));
    final area = raw['area'];
    final h3 =
        raw['h3_res9'] ??
  ...
  int? _intFromMap(Map<String, dynamic> raw, String key) {
    final value = raw[key];
    if (value is int) {
      return value;
    }
  ...
  num? _numFromMap(Map<String, dynamic> raw, String key) {
    final value = raw[key];
    if (value is num) {
      return value;
    }
  ...
  String? _stringFromMap(Map<String, dynamic> raw, String key) {
    final value = raw[key];
    if (value is String && value.trim().isNotEmpty) {
      return value.trim();
    }
  ...
  List<String> _stringListFromMap(Map<String, dynamic> raw, String key) {
    final value = raw[key];
    if (value is List) {
      return value
          .whereType<String>()

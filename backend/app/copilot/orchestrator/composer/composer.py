...
class DeterministicResponseComposer:
    def compose(self, execution_result: ExecutionResult) -> ComposedResponse:
        ...
        response_sections = self._response_sections(normalized_outputs, status, citations)
        evidence_summary = {
    ...
    def _response_sections(
        ...
        by_name = {output.tool_name: output.payload for output in outputs}
        valuation = by_name.get("valuation")
        comparable = by_name.get("comparable")
        explainability = by_name.get("explainability")
        market = by_name.get("market_insight")
        investment = by_name.get("investment")
        negotiation = by_name.get("negotiation")
        fairness = by_name.get("fairness")
        what_if = by_name.get("what_if")
        ...
        fair_price_source = (
            ...
            or (
                {
                    "fair_price": what_if["scenario_valuation"],
                    "confidence_level": what_if["confidence_level"],
                }
                if what_if is not None
                else None
            )
        )
        fair_price = fair_price_source.get("fair_price") if fair_price_source else None
        confidence = fair_price_source.get("confidence_level") if fair_price_source else None
        price_range = valuation.get("price_range") if valuation is not None else None

        strengths = [
            item["text"]
            for item in (investment or {}).get("strengths", [])
            if isinstance(item, dict) and isinstance(item.get("text"), str)
        ][:TOP_N_FEATURE_DRIVERS]
        risks = [
            item["text"]
            for item in (investment or {}).get("risks", [])
            if isinstance(item, dict) and isinstance(item.get("text"), str)
        ][:TOP_N_FEATURE_DRIVERS]
        negotiation_risks = [
            item["text"]
            for item in (negotiation or {}).get("risk_notes", [])
            if isinstance(item, dict) and isinstance(item.get("text"), str)
        ][:TOP_N_FEATURE_DRIVERS]
        ...
        if comparable is not None:
            comparable_count = comparable.get("comparable_count")
            comparable_ids = [
                item["comparable_id"]
                for item in comparable.get("comparables", [])
                if isinstance(item, dict) and isinstance(item.get("comparable_id"), str)
            ][:TOP_N_COMPARABLES]
        elif investment is not None:
            summary = investment.get("comparable_summary", {})
            comparable_count = summary.get("comparable_count")
            comparable_ids = [
                item for item in summary.get("comparable_ids", []) if isinstance(item, str)
            ][:TOP_N_COMPARABLES]
        elif negotiation is not None:
            summary = negotiation.get("comparable_summary", {})
            comparable_count = summary.get("comparable_count")
            comparable_ids = [
                item for item in summary.get("comparable_ids", []) if isinstance(item, str)
            ][:TOP_N_COMPARABLES]
        ...
        if market is not None:
            market_statements = [
                item["text"]
                for item in market.get("evidence_summary", {}).get("statements", [])
                if isinstance(item, dict) and isinstance(item.get("text"), str)
            ][:TOP_N_MARKET_ITEMS]

        recommended_action = None
        if investment is not None:
            recommended_action = investment.get("investment_position")
        elif negotiation is not None:
            recommended_action = negotiation.get("negotiation_position")
        elif fairness is not None:
            recommended_action = fairness.get("fairness_status")

        key_drivers = []
        if explainability is not None and isinstance(explainability.get("strongest_factors"), str):
            key_drivers.append(explainability["strongest_factors"])
        if investment is not None and isinstance(investment.get("investment_position_reason"), str):
            key_drivers.append(investment["investment_position_reason"])
        if negotiation is not None and isinstance(negotiation.get("negotiation_position_reason"), str):
            key_drivers.append(negotiation["negotiation_position_reason"])
        key_drivers.extend(strengths)

        return {
            "executive_summary": {
                "status": status.value,
                "primary_result": recommended_action,
                "source": "DETERMINISTIC_COMPOSER",
            },
            "fair_value": {
                "fair_price": fair_price,
                "price_range": price_range,
                "valuation_ids": citations.get("valuation_ids", []),
            },
            "confidence": {
                "level": confidence,
            },
            "key_drivers": key_drivers[:TOP_N_FEATURE_DRIVERS],
            "comparable_insights": {
                "comparable_count": comparable_count,
                "comparable_ids": comparable_ids,
            },
            "market_context": {
                "valuation_volume": market.get("valuation_volume") if market is not None else None,
                "comparable_density": (
                    market.get("comparable_density", {}).get("density_level")
                    if market is not None
                    else None
                ),
                "statements": market_statements,
            },
            "risks": (risks or negotiation_risks)[:TOP_N_FEATURE_DRIVERS],
            "recommended_action": recommended_action,
            "evidence_sources": {
                "tools": [output.tool_name for output in outputs],
                "valuation_ids": citations.get("valuation_ids", []),
                "comparable_ids": citations.get("comparable_ids", []),
                "unavailable_optional_citation_types": citations.get(

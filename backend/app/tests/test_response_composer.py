...
def test_v2_response_sections_are_derived_from_tool_evidence_only():
    comparable_payload = {
        "tool_name": "comparable",
        "valuation_id": "val_comparable",
        "comparable_count": 2,
        "comparables": [_comparable(1), _comparable(2)],
        "timestamp": _TIMESTAMP,
        "source": "TruthLayer",
    }
    response = DeterministicResponseComposer().compose(
        _execution(
            primary_intent=Intent.PROPERTY_EVALUATION,
            results=(
                _success(PlannedToolCall.VALUATION_TOOL, _valuation("val_eval", 1_250_000, "High"), 0),
                _success(PlannedToolCall.COMPARABLES_TOOL, comparable_payload, 1),
                _success(
                    PlannedToolCall.MARKET_INSIGHT_TOOL,
                    _market_insight(valuation_volume=4, density_level="High"),
                    2,
                ),
            ),
        )
    ...
    sections = response["frontend_payload"]["response_sections"]
    assert sections["fair_value"]["fair_price"] == 1_250_000
    assert sections["confidence"]["level"] == "High"
    assert sections["comparable_insights"]["comparable_count"] == 2
    assert sections["market_context"]["valuation_volume"] == 4
    assert sections["evidence_sources"]["tools"] == ["valuation", "comparable", "market_insight"]
